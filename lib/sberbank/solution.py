...
from lib.sberbank.utils import *
from lib.sberbank.solvers import *
...
class CuttingEdgeStrongGeneralAI(object):
    ...
    def __init__(self, solver_ids=None):
        self.train_path = sber_path('/var/sberbank/public_set/train')
        ...
        if solver_ids is not None:
            solver_ids = [i - 1 for i in solver_ids]
            solver_classes = [s for i, s in enumerate(all_solver_classes) if i in solver_ids]
        else:
            solver_ids = list(range(len(all_solver_classes)))
            solver_classes = all_solver_classes

        solver_classes = {solver_id: solver for solver_id, solver in zip(solver_ids, solver_classes)}
        self.solvers = self.solver_loading(solver_classes)
        self.clf_fitting()

    def solver_loading(self, solver_classes):
        solvers = {}
        for i, solver_class in solver_classes.items():
            solver_index = i + 1
            train_tasks = load_tasks(self.train_path, task_num=solver_index)
            solver_path = os.path.join(sber_path('/var/sberbank'), "models", "solver{}.pkl".format(solver_index))
