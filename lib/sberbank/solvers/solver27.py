...
class Solver(object):
    ...
    def load(self, path="data/models/solver27.pkl"):
        ...
        self.learn = language_model_learner(self.data, AWD_LSTM,pretrained=False,config=conf, drop_mult=0.7,
                                            pretrained_fnames=[self.model_name_to_save, self.dict_name_to_save], path=sber_path('/var/sberbank'))
    ...
    def generate(self, input_task="", seed="", n_words=300, no_unk=True, temperature=0.9, max_words=400):
        if input_task != "":
            topic = self.get_topic([input_task])[0]
            info = self.getinfo(topic)
            seed = info['Первая_фраза']

        text = self.learn.predict(seed, n_words=n_words, no_unk=no_unk, temperature=temperature)
        essay = post_prc(self.learn, text, temperature, n_words, max_words)
        try:
